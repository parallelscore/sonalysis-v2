...
const Analytics = () => {
  ...
  return (
    <div className="all-video">
            ...
            <div>
              {
                allUploadData?.data && allUploadData?.data?.map((item, id) => (
                  ...
                  <div className="table-row d-flex align-items-center p-3 mt-4">
                    ...
                    <div className="col-2">
                      <button className={item.analyzed ? "view" : "analyzing"} onClick={() => handleOpenModal(true)}>{item.status ? "View analytics" : "Analyzing"}</button>
                    </div>
      ...
      {openProgressModal && <UploadProgressModal setOpenProgressModal={setOpenProgressModal}  handleChangeTab={handleChangeTab}/>}
