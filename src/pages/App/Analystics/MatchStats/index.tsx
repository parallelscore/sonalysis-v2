    ...
    <div className="match-stat">
      ...
      <div className="d-flex flex-wrap">
        {actionUniqueArr.map((item) => (
          <div className={`card d-flex mr-4 p-3 text-black m-2 ${item==actionName&&"active-card"}`} onClick={()=>setActionName(item)}>
          ...
          <div className=" d-lg-flex flex-wrap">
            {filterAction.map((actionItem: any) => (
              <div
                ...
                onClick={() => handleVideoChange(Number(actionItem[3]))}
              >
                <img src={Football} alt="video" />
                <div>
                  <div>{actionItem[0]}</div>
                  <div>
                    {actionItem[2]} {actionItem[3]}` - {actionItem[4]}`
                  </div>
                  <div>
                    By: {actionItem[1]}
                    </div>
